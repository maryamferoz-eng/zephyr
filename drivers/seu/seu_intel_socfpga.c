...
#define SEU_ERROR_IRQ DT_IRQN(DT_NODELABEL(seu))
...
static int handle_data(uint32_t *data, uint8_t mask, struct seu_intel_socfpga_data *priv_seu_data)
{
	...
	for (int index = 0; index < CONFIG_SEU_MAX_CLIENT; index++) {
		if (priv_seu_data->seu_client_local.seu_isr_callback_enable[index] &&
...
static void seu_callback(uint32_t c_token, struct sip_svc_response *response)
{
	...
	priv = (struct seu_private_data *)response->priv_data;
...
static int seu_send_sip_svc(uint32_t *cmd_addr, uint32_t cmd_size, uint32_t *resp_addr,
			    uint32_t resp_size, struct seu_private_data *priv)
{
	...
	err = svc_client_open(priv->seu_data_ptr);
	if (err) {
	...
	trans_id = sip_svc_send(priv->seu_data_ptr->mailbox_smc_dev,
				priv->seu_data_ptr->mailbox_client_token, &request,
				seu_callback);
	if (trans_id < 0) {
...
sip_svc_client_close:
	err = svc_client_close(priv->seu_data_ptr);
	if (err) {
...
static int seu_intel_socfpga_init(const struct device *dev)
{
	...
	IRQ_CONNECT(SEU_ERROR_IRQ, SEU_PRIORITY, seu_irq_handler, DEVICE_DT_GET(DT_NODELABEL(seu)),
		    SEU_IRQ_FLAGS);
	irq_enable(SEU_ERROR_IRQ);
